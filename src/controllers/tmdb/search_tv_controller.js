...
export const search_tmdb_tv = async (req, res, next) => {
  ...
  const searchRoute = `${tmdb.base_url}/search/tv`;
  ...
  try {
    ...
    const watchlist = await findWatchlist(dec.userId, "tv");

    debug(`WATCHLIST DOCUMENT => ${watchlist}`);

    const savedMovies = [];
    let savedSeries = [];
    if (watchlist) savedSeries = watchlist.series || [];

    const responseData = await searchResults(response.data.results, lang, savedMovies, savedSeries, "tv");
